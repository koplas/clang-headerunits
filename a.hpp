...
template < class _InputIter,
 ...
 constexpr void advance(_InputIter& __i, _Distance __orig_n) {
...
template <class _InputIter, __enable_if_t<__has_input_iterator_category<_InputIter>::value, int> = 0>
[[__nodiscard__]]  constexpr _InputIter prev(_InputIter __it) {
...
template <class _Tp, size_t _Sz>
[[nodiscard]]  constexpr bool empty(const _Tp (&)[_Sz]) noexcept {
  return false;
}

template <class _Ep>
[[nodiscard]]  constexpr bool empty(initializer_list<_Ep> __il) noexcept {
  return __il.size() == 0;
}
...
template <class _Tp, size_t _Np>
 constexpr reverse_iterator<_Tp*> rbegin(_Tp (&__array)[_Np]) {
  return reverse_iterator<_Tp*>(__array + _Np);
}

template <class _Tp, size_t _Np>
 constexpr reverse_iterator<_Tp*> rend(_Tp (&__array)[_Np]) {
  return reverse_iterator<_Tp*>(__array);
}

template <class _Ep>
 constexpr reverse_iterator<const _Ep*> rbegin(initializer_list<_Ep> __il) {
  return reverse_iterator<const _Ep*>(__il.end());
}

template <class _Ep>
 constexpr reverse_iterator<const _Ep*> rend(initializer_list<_Ep> __il) {
  return reverse_iterator<const _Ep*>(__il.begin());
}

template <class _Cp>
 constexpr auto rbegin(_Cp& __c) -> decltype(__c.rbegin()) {
  return __c.rbegin();
}

template <class _Cp>
 constexpr auto rbegin(const _Cp& __c) -> decltype(__c.rbegin()) {
  return __c.rbegin();
}

template <class _Cp>
 constexpr auto rend(_Cp& __c) -> decltype(__c.rend()) {
  return __c.rend();
}

template <class _Cp>
 constexpr auto rend(const _Cp& __c) -> decltype(__c.rend()) {
  return __c.rend();
}

template <class _Cp>
 constexpr auto crbegin(const _Cp& __c) -> decltype(std::rbegin(__c)) {
  return std::rbegin(__c);
}

template <class _Cp>
 constexpr auto crend(const _Cp& __c) -> decltype(std::rend(__c)) {
  return std::rend(__c);
}
...
template <class _Cont>
 constexpr auto size(const _Cont& __c) noexcept(noexcept(__c.size())) -> decltype(__c.size()) {
  return __c.size();
}
...
public:
  ...
  constexpr  basic_string_view(const _CharT* __s, size_type __len) noexcept
      : __data_(__s),
        __size_(__len) {
  ...
  [[__nodiscard__]]  constexpr bool empty() const noexcept { return __size_ == 0; }
  ...
  constexpr  const_pointer data() const noexcept { return __data_; }
  ...
  constexpr  void swap(basic_string_view& __other) noexcept {
...
inline  constexpr pair<_InIter, _OutIter> __copy(_InIter, _Sent, _OutIter);
...
struct __copy_impl {
  ...
  struct _CopySegment {
    ...
    _OutIter& __result_;
     ...
     constexpr explicit _CopySegment(_OutIter& __result)
        : __result_(__result) {}
  ...
  template <class _Cp, bool _IsConst>
   pair<__bit_iterator<_Cp, _IsConst>, __bit_iterator<_Cp, false> >
...
template <class _InIter, class _Sent, class _OutIter>
pair<_InIter, _OutIter> inline  constexpr
...
inline  constexpr int __libcpp_popcount(unsigned long __x) noexcept {
  return __builtin_popcountl(__x);
}

inline  constexpr int __libcpp_popcount(unsigned long long __x) noexcept {
  return __builtin_popcountll(__x);
}

template <class _Tp>
[[__nodiscard__]]  constexpr int __popcount_impl(_Tp __t) noexcept {
  if constexpr (sizeof(_Tp) <= sizeof(unsigned int)) {
...
template <class _Tp>
[[__nodiscard__]]  constexpr int __popcount(_Tp __t) noexcept {
...
struct __optional_storage_base : __optional_destruct_base<_Tp> {
  ...
  using __base::__base;
   ...
   constexpr bool has_value() const noexcept { return this->__engaged_; }
...
struct __optional_storage_base<_Tp, true> {
  using value_type = _Tp;
  using __raw_type [[__gnu__::__nodebug__]] = remove_reference_t<_Tp>;
  __raw_type* __value_;
  ...
  template <class _UArg>
   ...
   constexpr bool has_value() const noexcept { return __value_ != nullptr; }

   constexpr value_type& __get() const& noexcept { return *__value_; }

   constexpr value_type&& __get() const&& noexcept { return std::forward<value_type>(*__value_); }
...
private:
  ...
  struct _CheckOptionalLikeConstructor {
    ...
    using __check_constructible_from_opt [[__gnu__::__nodebug__]] =
        _Or< is_constructible<_Tp, _Opt&>,
             ...
             is_convertible<_Opt const&&, _Tp> >;
    template <class _Up, class _Opt = optional<_Up>>
    using __check_assignable_from_opt [[__gnu__::__nodebug__]] =
...
public:
  ...
  template <class _Up = value_type,
   ...
   constexpr optional(_Up&& __v) : __base(in_place, std::forward<_Up>(__v)) {}

  template <class _Up, enable_if_t<_CheckOptionalArgsCtor<_Up>::template __enable_explicit<_Up>(), int> = 0>
   constexpr explicit optional(_Up&& __v) : __base(in_place, std::forward<_Up>(__v)) {}
  ...
  swap(optional& __opt) noexcept(is_nothrow_move_constructible_v<value_type> && is_nothrow_swappable_v<value_type>) {
   ...
   constexpr add_pointer_t<value_type const> operator->() const noexcept {
    ((void)0);
    return std::addressof(this->__get());
  }

   constexpr add_pointer_t<value_type> operator->() noexcept {
    ((void)0);
    return std::addressof(this->__get());
  }
...
template <
 ...
 constexpr bool operator==(const optional<_Tp>& __x, const optional<_Up>& __y) {
...
struct __cxx_atomic_base_impl {
 ...
 inline void __cxx_atomic_thread_fence(memory_order __order) noexcept {
  __c11_atomic_thread_fence(static_cast<__memory_order_underlying_t>(__order));
}

 inline void __cxx_atomic_signal_fence(memory_order __order) noexcept {
...
#pragma clang diagnostic ignored "-Wc++23-extensions"
 ...
 inline constexpr int __to_gcc_order(memory_order __order) {
 ...
 inline constexpr int __to_gcc_failure_order(memory_order __order) {
...
struct __atomic_base
{
  ...
  compare_exchange_weak(_Tp& __e, _Tp __d, memory_order __s, memory_order __f) volatile noexcept
   ...
   bool compare_exchange_weak(_Tp& __e, _Tp __d, memory_order __s, memory_order __f) noexcept
      __attribute__((__diagnose_if__(__f == memory_order_release || __f == memory_order_acq_rel, "memory order argument to atomic operation is invalid", "warning"))) {
    return std::__cxx_atomic_compare_exchange_weak(std::addressof(__a_), std::addressof(__e), __d, __s, __f);
  }
   bool
  compare_exchange_strong(_Tp& __e, _Tp __d, memory_order __s, memory_order __f) volatile noexcept
   ...
   bool compare_exchange_strong(_Tp& __e, _Tp __d, memory_order __s, memory_order __f) noexcept
      __attribute__((__diagnose_if__(__f == memory_order_release || __f == memory_order_acq_rel, "memory order argument to atomic operation is invalid", "warning"))) {
    return std::__cxx_atomic_compare_exchange_strong(std::addressof(__a_), std::addressof(__e), __d, __s, __f);
  }
   bool
...
struct __atomic_waitable_traits<__atomic_base<_Tp, _IsIntegral> > {
  ...
  static  const __cxx_atomic_impl<_Tp>*
...
template <class _Tp>
 bool atomic_is_lock_free(const atomic<_Tp>* __o) noexcept {
  return __o->is_lock_free();
}
...
template <class _Tp>
 void atomic_store(volatile atomic<_Tp>* __o, typename atomic<_Tp>::value_type __d) noexcept {
  __o->store(__d);
}

template <class _Tp>
 void atomic_store(atomic<_Tp>* __o, typename atomic<_Tp>::value_type __d) noexcept {
  __o->store(__d);
}
...
template <class _Tp>
 _Tp atomic_load(const volatile atomic<_Tp>* __o) noexcept {
  return __o->load();
}

template <class _Tp>
 _Tp atomic_load(const atomic<_Tp>* __o) noexcept {
  return __o->load();
}
...
template <class _Tp>
 _Tp atomic_load_explicit(const volatile atomic<_Tp>* __o, memory_order __m) noexcept
    __attribute__((__diagnose_if__(__m == memory_order_release || __m == memory_order_acq_rel, "memory order argument to atomic operation is invalid", "warning"))) {
  return __o->load(__m);
}

template <class _Tp>
 _Tp atomic_load_explicit(const atomic<_Tp>* __o, memory_order __m) noexcept
    __attribute__((__diagnose_if__(__m == memory_order_release || __m == memory_order_acq_rel, "memory order argument to atomic operation is invalid", "warning"))) {
  return __o->load(__m);
}
...
template <class _Tp>
 _Tp atomic_exchange(volatile atomic<_Tp>* __o, typename atomic<_Tp>::value_type __d) noexcept {
  return __o->exchange(__d);
}

template <class _Tp>
 _Tp atomic_exchange(atomic<_Tp>* __o, typename atomic<_Tp>::value_type __d) noexcept {
  return __o->exchange(__d);
}
...
template <class _Tp>
 _Tp atomic_fetch_add(atomic<_Tp>* __o, typename atomic<_Tp>::difference_type __op) noexcept {
  return __o->fetch_add(__op);
}
...
template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_and(volatile atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_and(__op);
}

template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_and(atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_and(__op);
}
...
template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_or(volatile atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_or(__op);
}

template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_or(atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_or(__op);
}
...
template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_xor(volatile atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_xor(__op);
}

template <class _Tp, __enable_if_t<is_integral<_Tp>::value && !is_same<_Tp, bool>::value, int> = 0>
 _Tp atomic_fetch_xor(atomic<_Tp>* __o, typename atomic<_Tp>::value_type __op) noexcept {
  return __o->fetch_xor(__op);
}
...
inline  int __libcpp_mutex_destroy(__libcpp_mutex_t* __m) { return pthread_mutex_destroy(__m); }
...
inline  int __libcpp_condvar_signal(__libcpp_condvar_t* __cv) { return pthread_cond_signal(__cv); }
...
inline  int __libcpp_thread_join(__libcpp_thread_t* __t) { return pthread_join(*__t, nullptr); }

inline  int __libcpp_thread_detach(__libcpp_thread_t* __t) { return pthread_detach(*__t); }

inline  void __libcpp_thread_yield() { sched_yield(); }
...
inline  void* __libcpp_tls_get(__libcpp_tls_key __key) { return pthread_getspecific(__key); }
...
struct __atomic_waitable_traits<atomic_flag> {
  static  bool __atomic_load(const atomic_flag& __a, memory_order __order) {
  ...
  static  const __cxx_atomic_impl<bool>*
  __atomic_contention_address(const atomic_flag& __a) {
    return std::addressof(__a.__a_);
  }

  static  const volatile __cxx_atomic_impl<bool>*
  __atomic_contention_address(const volatile atomic_flag& __a) {
    return std::addressof(__a.__a_);
  }
};

inline  bool atomic_flag_test(const volatile atomic_flag* __o) noexcept { return __o->test(); }

inline  bool atomic_flag_test(const atomic_flag* __o) noexcept { return __o->test(); }
...
inline  bool atomic_flag_test_and_set(atomic_flag* __o) noexcept { return __o->test_and_set(); }
...
inline  void atomic_flag_clear(volatile atomic_flag* __o) noexcept { __o->clear(); }

inline  void atomic_flag_clear(atomic_flag* __o) noexcept { __o->clear(); }
...
public:
   ...
   istream_iterator(istream_type& __s) : __in_stream_(std::addressof(__s)) {
   ...
   const _Tp* operator->() const { return std::addressof((operator*())); }
...
inline  bool operator==(const istream_iterator<_Tp, _CharT, _Traits, _Distance>& __x,
...
private:
  ...
  class __proxy {
    ...
    streambuf_type* __sbuf_;
     explicit __proxy(char_type __c, streambuf_type* __s) : __keep_(__c), __sbuf_(__s) {}
...
public:
   ...
   istreambuf_iterator(istream_type& __s) noexcept : __sbuf_(__s.rdbuf()) {}
   istreambuf_iterator(streambuf_type* __s) noexcept : __sbuf_(__s) {}
   istreambuf_iterator(const __proxy& __p) noexcept : __sbuf_(__p.__sbuf_) {}

   char_type operator*() const { return static_cast<char_type>(__sbuf_->sgetc()); }
   istreambuf_iterator& operator++() {
    __sbuf_->sbumpc();
    return *this;
  }
   __proxy operator++(int) { return __proxy(__sbuf_->sbumpc(), __sbuf_); }
